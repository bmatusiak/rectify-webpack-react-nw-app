...
//the tray icon, and the menu app plugins add to.
//
...
plugin.consumes = ['app', 'server', 'window', 'lifecycle'];
plugin.provides = ['tray'];
async function plugin(imports, register, config) {
    var { app, server, window: win, lifecycle } = imports;
    ...
    //rebuilt whole rather than patched: plugins come and go on every reload,
    //and removing by index is how menus end up with the wrong item on them
    function rebuild() {
        ...
        menu.append(new nw.MenuItem({
            ...
            click: function () { nw.Shell.openExternal(server.url); }
        }));
    ...
    var api = {
        ...
        //tooltip, icon, submenu, key, modifiers. returns a handle, so whoever
        //added an item can take it back when they go
        ...
        //called once the server is up, when there is a url to put in the tooltip
        start: function () {
            try {
                tray = new nw.Tray({
                ...
                tray.tooltip = app.appPackage.title + ' — ' + server.url;//see nw.js issue 1903
